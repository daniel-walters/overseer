...
describe("mirrorReconciler — epic create", () => {
  it("creates one epic for an opted-in PRD with no backref and writes the key back", () => {
    ...
    const delta = reconciler.reconcile(board(prd("auth", "backlog", "Auth")));
  ...
  it("does nothing for a PRD with no jira block", () => {
    const seam = new FakeJiraSeam();
    const { reconciler } = harness(seam);
    // No opt-in seeded for "private" ⇒ readMirror returns {}.

    const delta = reconciler.reconcile(board(prd("private", "in-progress")));

    expect(seam.calls).toEqual([]);
    expect(delta).toEqual({ created: [], transitioned: [] });
  });

  it("never creates a second epic when the backref is already present", () => {
    const seam = new FakeJiraSeam();
    seam.epics.set("DS-9", "To Do");
    const { reconciler, set, epicKeyOf } = harness(seam);
    set("auth", { optIn: optIn({ board: "34" }), epicKey: "DS-9" });

    const delta = reconciler.reconcile(board(prd("auth", "backlog")));
  ...
  it("derives the project from the board, or honours an explicit project override", () => {
    const seam = new FakeJiraSeam();
    seam.projectByBoard.set("34", "DS");
    ...
    reconciler.reconcile(board(prd("byBoard", "backlog"), prd("byOverride", "backlog")));
  ...
  it("falls back to the configured default_board when the opt-in names none", () => {
    const seam = new FakeJiraSeam();
    ...
    reconciler.reconcile(board(prd("auth", "backlog")));

    expect(seam.calls).toContain("resolveProject:99");
    expect(seam.created.map((c) => c.project)).toEqual(["DEF"]);
  });

  it("logs a no-op and creates nothing when no board can be resolved", () => {
    const seam = new FakeJiraSeam();
  ...
  it("isolates a seam failure to its own PRD, still reconciling the rest", () => {
    const seam = new FakeJiraSeam();
    seam.projectByBoard.set("34", "DS");
...
describe("mirrorReconciler — epic status self-heal", () => {
  it("transitions a freshly-created epic to match a PRD already in progress", () => {
    const seam = new FakeJiraSeam();
    seam.projectByBoard.set("34", "DS");
    ...
    const delta = reconciler.reconcile(board(prd("auth", "in-progress")));
  ...
  it("leaves a backlog PRD's fresh epic in To Do (no needless transition)", () => {
    const seam = new FakeJiraSeam();
    seam.projectByBoard.set("34", "DS");
  ...
  it("self-heals a linked epic toward the board when a human moved it", () => {
    const seam = new FakeJiraSeam();
    ...
    const delta = reconciler.reconcile(board(prd("auth", "done")));

    expect(delta.transitioned).toEqual([{ key: "DS-9", to: "Done" }]);
    expect(seam.epics.get("DS-9")).toBe("Done");
  });

  it("does not transition an epic already at its target status", () => {
    const seam = new FakeJiraSeam();
  ...
  it("degrades an unavailable target status to a logged no-op, never a throw", () => {
    const seam = new FakeJiraSeam();
    seam.epics.set("DS-9", "To Do");
  ...
  it("terminates the backref write-back self-scan as a zero-JIRA no-op", () => {
    const seam = new FakeJiraSeam();
    seam.projectByBoard.set("34", "DS");
    seam.legalStatuses.add("In Progress");
    const { reconciler, set } = harness(seam);
    ...
    const second = reconciler.reconcile(b);
